...
class PLCWorker(QThread):
    ...
    def run(self):
        temps_ecoule = 0
        tags_plc = [tag["plc"] for tag in self.config["tags"]]
...
class MyWindow(QMainWindow):
    ...
    def start_logix_recorder(self):
        if self.worker and self.worker.isRunning():
            ...
            self.csv_manager()
            self.worker = PLCWorker(configuration, self.current_csv_path)
            self.worker.log_signal.connect(self.ui.logger.append)
            self.worker.finished.connect(self.reset_interface_after_stop)
            self.worker.start()
            self.ui.button_start_stop.setText("Stop")
            self.toggle_ui_elements(False)
            self.ui.logger.append("Démarrage de l'enregistrement...")

    def reset_interface_after_stop(self):
        self.ui.button_start_stop.setText("Start")
        self.toggle_ui_elements(True)
        self.ui.logger.append("Système arrêté.")

    def toggle_ui_elements(self, state):
        self.ui.type_plc.setEnabled(state)
        self.ui.adressip.setEnabled(state)
        self.ui.slot_number.setEnabled(state)
        self.ui.temps_scan.setEnabled(state)
        self.ui.csv_file_text.setEnabled(state)
        self.ui.csv_file_icon_button.setEnabled(state)
        self.ui.add_line_bp.setEnabled(state)
        self.ui.remove_line_bp.setEnabled(state)
        self.ui.plccsvtable.setEnabled(state)
        self.ui.actionOuvrir.setEnabled(state)
    ...
    def csv_manager(self):
        if configuration["csv_file_adress"] != "":
            datetime_now = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.current_csv_path = f"{configuration['csv_file_adress']}/plc_data_{datetime_now}.csv"
            with open(self.current_csv_path, 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                header = ["Timestamp"] + [tag["csv"] for tag in configuration["tags"]]
